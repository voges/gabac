...
off_t File::tell() const{
    ...
    if (offset == -1)
    {
        GABAC_DIE("ftell failed on file: " + m_path);
    }
    return offset;
}
...
void File::close(){
    if (m_fp != nullptr)
    {
        ...
        if (rc != 0)
        {
            GABAC_DIE("Failed to close file: " + m_path);
        }
        m_fp = nullptr;
    }
    else
...
void File::open(
    ...
    if (m_fp == nullptr)
    {
        GABAC_DIE("Failed to open file: " + m_path);
    }
#endif
    ...
    if (fd == -1)
    {
        GABAC_DIE("Failed to get file descriptor of file: " + m_path);
    }
    ...
    {
        GABAC_DIE("Not a regular file: " + m_path);
    }
#endif
}
...
void File::seek(
    ...
    if (rc != 0)
    {
        GABAC_DIE("fseek failed on file: " + m_path);
    }
}
