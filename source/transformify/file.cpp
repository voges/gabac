...
off_t File::tell() const
{
    ...
    if (offset == -1)
    {
        TRANSFORMIFY_DIE("ftell failed on file: " + m_path);
    }
    return offset;
}
...
void File::close()
{
    if (m_fp != nullptr)
    {
        ...
        if (rc != 0)
        {
            TRANSFORMIFY_DIE("Failed to close file: " + m_path);
        }
        m_fp = nullptr;
    }
    else
...
void File::open(const std::string& path, const char* const mode)
{
    ...
    if (err != 0)
    {
        TRANSFORMIFY_DIE("Failed to open file: " + m_path);
    }
#else
    ...
    if (fd == -1)
    {
        TRANSFORMIFY_DIE("Failed to get file descriptor of file: " + m_path);
    }
    ...
    {
        TRANSFORMIFY_DIE("Not a regular file: " + m_path);
    }
#else
...
void File::seek(off_t offset, int whence)
{
    ...
    if (rc != 0)
    {
        TRANSFORMIFY_DIE("fseek failed on file: " + m_path);
    }
}
